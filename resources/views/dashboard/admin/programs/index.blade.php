<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between">
            <form method="GET" action="{{ route('admin.program.index') }}" class="flex items-center gap-4 pt-2">
                <input type="text" name="search" value="{{ request()->get('search') }}"
                    placeholder="Search newss by title" class="py-2 px-4 border rounded-lg">
                ...
                @foreach ($programs as $program)
                    <div class="item-card flex flex-row justify-between items-center mb-6">
                        ...
                        <div class="hidden md:flex flex-row items-center gap-x-3">
                            <!-- Button for Modal -->
                            <button onclick="openModal('modal-{{ $program->ID_program }}')"
                            ...
                            <form action="{{ route('admin.program.destroy', $program->ID_program) }}" method="POST"
                    ...
                    <!-- Modal -->
                    <div id="modal-{{ $program->ID_program }}"
                        ...
                        <div class="bg-white p-4 rounded-lg shadow-lg w-full max-w-md">
                            ...
                            <div class="flex items-center justify-between mb-4 border-b pb-2">
                                ...
                                <button onclick="closeModal('modal-{{ $program->ID_program }}')"
                            ...
                            <div class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-800">
                                        {{ \Carbon\Carbon::parse($program->Execution_Date)->format('d M Y') }}</p>
                                </div>
                            ...
                            <div class="mt-4 flex justify-center">
                                @if ($program->program_Image)
                                    <img src="{{ asset('storage/' . $program->program_Image) }}"
                                        alt="{{ $program->program_Name }}"
                                        class="rounded-lg object-cover mx-auto shadow-md w-full h-40">
                                @else
                                    <div
                                        class="w-full h-40 bg-gray-300 rounded-lg flex items-center justify-center text-gray-500">
                                        No Image Available
                                    </div>
                                @endif
                            </div>
