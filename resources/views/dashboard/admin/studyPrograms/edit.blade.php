            ...
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.studyProgram.update', $studyProgram->ID_study_program) }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input id="study_program_Name" class="block mt-1 w-full" type="text" name="study_program_Name" :value="old('study_program_Name', $studyProgram->study_program_Name)" required />
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="classrooms" class="block mt-1 w-full" type="number" name="classrooms" :value="old('classrooms', $studyProgram->classrooms)" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="lecturers" :value="__('Lecturers')" />
                        <x-text-input id="lecturers" class="block mt-1 w-full" type="number" name="lecturers" :value="old('lecturers', $studyProgram->lecturers)" />
                    </div>
                    <div class="mt-4">
                        <x-input-label for="national_accreditation" :value="__('National Accreditation')" />
                        <x-text-input id="national_accreditation" class="block mt-1 w-full" type="text" name="national_accreditation" :value="old('national_accreditation')" />
                    </div>
                    <div class="mt-4">
                        <x-input-label for="international_accreditation" :value="__('International Accreditation')" />
                        <x-text-input id="international_accreditation" class="block mt-1 w-full" type="text" name="international_accreditation" :value="old('international_accreditation')" />
                    </div>
                    <div class="mt-4">
